...
MU_TEST(test_mat4_perspective)
{
	...
	mu_assert(nearly_equal(result[0], cot_half_fov / aspect, 0.0001f),
			  "perspective m[0,0] X scale");
	...
	// result[14] = (n * f) / (n - f)
	mu_assert(nearly_equal(result[14], (n * f) / (n - f), 0.0001f),
			  "perspective m[3,2] depth offset");
